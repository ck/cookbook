...
Check the @Merb.config@ in the @init.rb@ file:
...
First, let's add the MerbAuth gems to @config/init.rb@:
...
We put it under the merb-more section, since that's where the gems are maintained since version 1.0.
...
The astute reader also noticed that we set user class explicit. If we don't do that, MerbAuth assumes
our user class is @User@ and creates a @users@ table.

We also need to define our authentication strategies in @lib/authentication/setup.rb@ and make sure
merb-auth uses our @Cook@ model as the user class. For now we use standard password forms and OpenID:
...
TODO: Somehow it still creates the @users@ table, even through the user class is set to @Cook@ (checked in interactive Merb).
...
And while we're touching the controller, we switch to action arguments (merb-more/merb-action-args).
